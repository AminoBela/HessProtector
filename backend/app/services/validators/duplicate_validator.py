...
class DuplicateValidator(TransactionValidator):
    ...
    def _check(self, transaction, user_id: int, context: dict) -> tuple[bool, str]:
        ...
        recent_txs = self.transaction_repo.get_by_date_range(start_date, end_date, user_id)
        
        for tx in recent_txs:
            if (tx['label'] == transaction.label and
                tx['amount'] == transaction.amount and
                tx['type'] == transaction.type and
                tx['date'] == transaction.date):
                return False, f"Duplicate transaction detected: {transaction.label} ({transaction.amount}€)"
