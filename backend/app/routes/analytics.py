...
def get_monthly_analytics(year: str, month: str, current_user: User = Depends(get_current_user)):
    ...
    # Get Limits
    c.execute("SELECT category, amount FROM budget_limits WHERE user_id=?", (current_user['id'],))
...
def generate_audit(req: AuditRequest, current_user: User = Depends(get_current_user)):
    ...
    {{
        ...
        "tips": [
            {{ "icon": "☕", "tip": "Make coffee at home." }},
            {{ "icon": "📉", "tip": "Cancel Netflix if you don't watch it." }},
             {{ "icon": "💰", "tip": "Put 50€ in savings immediately." }}
        ]
    }}
    """
    ...
    for model_name in MODELS_TO_TRY:
        try:
            ...
            return {"analysis": parsed} # Return Object, not string
