...
class PantryRepository(BaseRepository):
    ...
    def get_all(self, user_id: int) -> List[dict]:
        c = self.conn.cursor()
        c.execute(
            "SELECT * FROM pantry WHERE user_id=? ORDER BY expiry ASC", (user_id,)
        )
...
class RecurringRepository(BaseRepository):
    ...
    def get_all(self, user_id: int) -> List[dict]:
        c = self.conn.cursor()
        c.execute(
            "SELECT * FROM recurring WHERE user_id=? ORDER BY day ASC", (user_id,)
        )
        return self._rows_to_dicts(c.fetchall())
...
class GoalsRepository(BaseRepository):
    ...
    def get_all(self, user_id: int) -> List[dict]:
        c = self.conn.cursor()
        c.execute(
            "SELECT * FROM goals WHERE user_id=? ORDER BY deadline ASC", (user_id,)
        )
        return self._rows_to_dicts(c.fetchall())
