    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
            <div className="md:col-span-4 space-y-8">
                <Card className={`h-fit border-0 ${cardGlass}`}><CardHeader><CardTitle className="text-sm uppercase tracking-widest text-zinc-400">{t.addTitle}</CardTitle></CardHeader><CardContent className="space-y-4"><Input placeholder={t.labelPlace} className={inputStyle} value={recForm.label} onChange={(e: any) => setRecForm({ ...recForm, label: e.target.value })} /><Input type="number" placeholder="Montant (€)" className={inputStyle} value={recForm.amount} onChange={(e: any) => setRecForm({ ...recForm, amount: e.target.value })} /><div className="grid grid-cols-2 gap-2"><Input type="number" placeholder="Jour (1-31)" className={inputStyle} value={recForm.day} onChange={(e: any) => setRecForm({ ...recForm, day: e.target.value })} /><select className={selectStyle} value={recForm.type} onChange={(e: any) => setRecForm({ ...recForm, type: e.target.value })}><option value="Fixe">{t.fixe}</option><option value="Abonnement">{t.sub}</option></select></div><Button onClick={handleAddRec} className="w-full bg-purple-600 hover:bg-purple-500 h-12 rounded-xl font-bold">{t.planBtn}</Button></CardContent></Card>
                <Card className={`border-0 ${cardGlass}`}><CardHeader><CardTitle className="text-sm uppercase text-zinc-400">{t.totalTitle}</CardTitle></CardHeader><CardContent><div className="text-4xl font-black">{data.monthly_burn}€</div><p className="text-xs text-zinc-500 mt-2">{t.desc}</p></CardContent></Card>
            </div>
