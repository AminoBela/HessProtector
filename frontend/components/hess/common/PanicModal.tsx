  ...
  const handlePanic = async () => {
    ...
    try {
      const res = await ApiService.post(
        "/smart-prompt",
        { type: "emergency", budget: 0, days: 5 },
        token || "",
      );
      setPlan(res.prompt);
    } catch (e) {
      ...
      >
        <DialogHeader>
          <DialogTitle className="text-center flex flex-col items-center gap-2">
            ...
            <span className="text-xs uppercase font-bold tracking-[0.5em] opacity-50">
              {t.subtitle}
            </span>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="mt-4">
          {loading && (
            <div className="text-center py-10 space-y-4">
              ...
              <p className="font-bold text-rose-400 animate-pulse">
                {t.loading}
              </p>
            </div>
          )}
          ...
          {plan && (
            ...
            >
              <div className="bg-rose-500/10 p-4 rounded-xl border border-rose-500/20 text-center">
                <p className="text-lg font-black text-rose-500 italic">
                  &quot;{plan.motivational_speech}&quot;
                </p>
              </div>
