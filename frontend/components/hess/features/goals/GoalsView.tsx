...
export function GoalsView({ data, goalForm, setGoalForm, handleAddGoal, handleDeleteGoal, language, theme }: GoalsViewProps) {
    ...
    const infoBoxStyle = isLight ? "bg-indigo-50 border-indigo-100 text-indigo-700" : "bg-indigo-900/20 border-indigo-500/20 text-indigo-300";
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-12 gap-8">
            <div className="md:col-span-4">
                <Card className={`h-fit border-0 ${cardGlass}`}>
                    <CardHeader>
                        <CardTitle className={`text-sm uppercase tracking-widest ${cardTitleColor}`}>{t.goals.newGoal}</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        ...
                        <Input type="number" placeholder={`${t.goals.target} (€)`} className={inputStyle} value={goalForm.target} onChange={(e: any) => setGoalForm({ ...goalForm, target: e.target.value })} />
                        <Input type="number" placeholder={`${t.goals.saved} (€)`} className={inputStyle} value={goalForm.saved} onChange={(e: any) => setGoalForm({ ...goalForm, saved: e.target.value })} />
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1">
                                ...
                                <Select value={goalForm.priority} onValueChange={(val) => setGoalForm({ ...goalForm, priority: val })}>
                        ...
                        <Button onClick={handleAddGoal} className="w-full bg-indigo-600 hover:bg-indigo-500 h-14 rounded-xl font-bold text-lg mt-4">{t.goals.add}</Button>
                    </CardContent>
                </Card>
            </div>
