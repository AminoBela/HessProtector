...
export function SetupWizard({ onFinish, bg }: SetupWizardProps) {
  ...
  const cardGlass = "card-glass card-glass-dark";

  const inputStyle =
    "bg-zinc-950/60 border-white/10 text-white placeholder:text-zinc-500 focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 backdrop-blur-xl transition-all h-12 rounded-xl px-4 font-medium shadow-inner";
  const selectStyle =
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 text-white font-sans relative overflow-hidden">
      {bg}
      <motion.div
      ...
      >
        <Card
          className={`border-0 ${cardGlass} shadow-2xl rounded-3xl overflow-hidden`}
        >
          <CardHeader className="pt-10 pb-2">
            <div className="flex justify-center mb-4">
              <ShieldCheck className="w-12 h-12 text-emerald-400" />
            </div>
            <CardTitle className="text-center text-3xl font-black uppercase tracking-widest bg-clip-text text-transparent bg-gradient-to-r from-white to-zinc-500">
              {setupStep === 0 ? t.step1 : setupStep === 1 ? t.step2 : t.step3}
            </CardTitle>
          </CardHeader>
              ...
              <div className="text-center space-y-6">
                <div className="relative w-full">
                  <Input
                    type="number"
                    placeholder="0"
                    className="text-center text-6xl font-black h-24 bg-transparent border-none text-white w-full focus-visible:ring-0 placeholder:text-zinc-800"
                    value={setupBalance}
                    onChange={(e) => setSetupBalance(e.target.value)}
                    autoFocus
                  />
                  <span className="absolute right-10 top-6 text-zinc-600 text-3xl font-light">
                    €
                  </span>
                </div>
                <Button
                  ...
                  className="w-full h-14 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-2xl text-lg"
                >
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      className="h-12 w-12 bg-emerald-600 hover:bg-emerald-500 rounded-xl"
                    >
                      <Plus />
                    </Button>
                  </div>
                </div>
                <ScrollArea className="h-40 rounded-xl border border-white/5 bg-black/20 p-2">
                  {setupBills.map((b, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center text-sm p-3 bg-white/5 rounded-lg mb-2 last:mb-0"
                    >
            ...
            {setupStep === 2 && (
              <div className="space-y-6">
                ...
                <Button
                  ...
                  className="w-full h-14 bg-emerald-500 text-white hover:bg-emerald-400 font-black rounded-2xl text-xl animate-pulse"
                >
                  {t.start}
                </Button>
              </div>
