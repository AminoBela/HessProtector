        ...
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-12 gap-8">
            ...
            <div className="md:col-span-8">
                <motion.div variants={item} className="h-full">
                    <Card className={`border-0 ${cardGlass} h-full`}><CardHeader className="flex flex-row justify-between items-center"><CardTitle className={`text-xl uppercase font-black tracking-widest ${isLight ? 'text-slate-400' : 'text-white'}`}>{t.sidebar.recurring}</CardTitle><Badge className={isLight ? "bg-emerald-600/10 text-emerald-800" : "bg-white/10 text-white px-3 py-1"}>{format(today, 'MMMM yyyy', { locale: locale })}</Badge></CardHeader><CardContent>
                            ...
                            <div className={`min-w-[700px] grid grid-cols-7 gap-1 p-4 rounded-2xl border ${isLight ? 'bg-slate-50 border-emerald-900/5' : 'bg-black/20 border-white/5'}`}>
                                ...
                                {Array.from({ length: currentMonthDays }).map((_, i) => {
                                    ...
                                    const dayBg = isLight
                                        ...
                                        : (isToday ? 'bg-purple-500/10 border-purple-500/30' : 'bg-zinc-800/50 hover:bg-zinc-700/50 border-zinc-700/50');
