            ...
            <div className="md:col-span-4 space-y-8">
                ...
                <Card className={`h-fit border-0 ${cardGlass} p-6`}>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <Button onClick={handleAddPantry} className="w-full bg-cyan-500 hover:bg-cyan-400 h-14 rounded-xl font-black text-white text-lg shadow-lg hover:scale-105 transition-all">{t.pantry.add}</Button>
                    </CardContent>
                </Card>
            </div>
                                ...
                                {(data?.pantry || []).length === 0 ? <div className="col-span-2 text-center text-zinc-600 py-10 italic">{t.pantry.empty}</div> :
                                    (data?.pantry || []).map((p: any) => (
                                        <motion.div variants={item} key={p.id}>
                                            <div className={`flex justify-between items-center p-5 rounded-2xl border transition-all group ${isLight ? 'bg-white/40 border-emerald-900/5 hover:bg-white/80' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
