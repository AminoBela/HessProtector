...
import { Bot, TrendingUp, TrendingDown, Wallet, PiggyBank, Sparkles, Check, Quote, ArrowUp, ArrowDown, Tag } from "lucide-react"
...
export function AnalyticsView({ language, theme, activeTab }: AnalyticsViewProps) {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (
                <div className={`p-4 rounded-2xl border ${isLight ? 'bg-white border-emerald-900/10' : 'bg-zinc-950 border-white/10'}`}>
                    {label && <p className={`mb-2 text-xs font-bold uppercase tracking-wider ${isLight ? 'text-slate-500' : 'text-zinc-500'}`}>{label}</p>}
                    {payload.map((p: any, index: number) => (
                        <div key={index} className="flex items-center gap-3 mb-1 last:mb-0">
                            ...
                            <span className={`text-sm font-medium ${isLight ? 'text-slate-600' : 'text-zinc-300'}`}>{p.name}:</span>
                            <span className={`text-sm font-black font-mono ${isLight ? 'text-slate-900' : 'text-white'}`}>{p.value.toFixed(2)}€</span>
                        </div>
                    ))}
                </div>
            );
        ...
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-8 pb-20">
            ...
            {audit && (
                ...
                >
                    ...
                    <Card className="relative z-10 border-0 bg-transparent shadow-none p-6">
                        ...
                        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start mb-8 border-b border-white/5 pb-8">
                            ...
                            <div className="relative w-32 h-32 shrink-0 flex items-center justify-center">
                                <svg className="w-full h-full transform -rotate-90">
                                    <circle cx="64" cy="64" r="56" fill="transparent" stroke={isLight ? "#e2e8f0" : "#27272a"} strokeWidth="8" />
                                    <circle cx="64" cy="64" r="56" fill="transparent" stroke="currentColor" strokeWidth="8" strokeDasharray={351} strokeDashoffset={351 - (351 * audit.score) / 10} className={`${getScoreColor(audit.score)} transition-all duration-1000 ease-out`} strokeLinecap="round" />
                                </svg>
                                <div className="absolute inset-0 flex flex-col items-center justify-center">
                                    <span className={`text-4xl font-black ${getScoreColor(audit.score)}`}>{audit.score}</span>
                                    <span className="text-xs uppercase font-bold opacity-50">Score</span>
                                </div>
                            </div>

                            {/* Title & Roast */}
                            <div className="flex-1 text-center md:text-left">
                                <h2 className={`text-3xl font-black uppercase tracking-tight mb-2 ${isLight ? 'text-slate-900' : 'text-white'}`}>{audit.title}</h2>
                                <p className={`text-lg leading-relaxed ${isLight ? 'text-slate-600' : 'text-zinc-400'}`}>"{audit.roast}"</p>
                            </div>
                        </div>

                        {/* Middle Section: Pros vs Cons */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                            <div className={`p-6 rounded-2xl ${isLight ? 'bg-emerald-50/50 border border-emerald-100' : 'bg-emerald-900/10 border border-emerald-500/20'}`}>
                                ...
                                <ul className="space-y-3">
                                    {audit.pros.map((pro, i) => (
                                        <li key={i} className={`flex items-start gap-2 text-sm font-medium ${isLight ? 'text-emerald-900' : 'text-emerald-100'}`}>
                            ...
                            <div className={`p-6 rounded-2xl ${isLight ? 'bg-rose-50/50 border border-rose-100' : 'bg-rose-900/10 border border-rose-500/20'}`}>
                                ...
                                <ul className="space-y-3">
                                    {audit.cons.map((con, i) => (
                                        <li key={i} className={`flex items-start gap-2 text-sm font-medium ${isLight ? 'text-rose-900' : 'text-rose-100'}`}>
                        ...
                        {/* Bottom Section: Tips */}
                        <div>
                            <h3 className={`text-sm font-bold uppercase tracking-widest opacity-50 mb-4 ${isLight ? 'text-slate-900' : 'text-white'}`}>Conseils du Coach</h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                {audit.tips.map((tip, i) => (
                                    <div key={i} className={`p-4 rounded-xl border flex items-start gap-4 ${isLight ? 'bg-white border-slate-100' : 'bg-zinc-900/50 border-white/5'}`}>
                                        <div className="text-2xl">{tip.icon}</div>
                                        <p className={`text-sm font-medium leading-relaxed ${isLight ? 'text-slate-700' : 'text-zinc-300'}`}>{tip.tip}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <motion.div variants={item} className="h-full relative">
                    ...
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        <div><p className="text-xs font-bold uppercase text-emerald-500 mb-1 tracking-wider">{t.analytics?.kpi.income || "Revenus"}</p><p className={`text-3xl font-black ${isBlurred ? 'blur-md select-none' : ''}`}>{stats.income.toFixed(2)}€</p></div>
                ...
                <motion.div variants={item} className="h-full relative">
                    <div className={`absolute inset-0 border-0 ${cardGlass} z-0`} />
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        <div><p className="text-xs font-bold uppercase text-rose-500 mb-1 tracking-wider">{t.analytics?.kpi.expense || "Dépenses"}</p><p className={`text-3xl font-black ${isBlurred ? 'blur-md select-none' : ''}`}>{stats.expense.toFixed(2)}€</p></div>
                ...
                <motion.div variants={item} className="h-full relative">
                    <div className={`absolute inset-0 border-0 ${cardGlass} z-0`} />
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        <div><p className="text-xs font-bold uppercase text-blue-500 mb-1 tracking-wider">{t.analytics?.kpi.net || "Net"}</p><p className={`text-3xl font-black ${stats.net >= 0 ? 'text-emerald-500' : 'text-rose-500'} ${isBlurred ? 'blur-md select-none' : ''}`}>{stats.net.toFixed(2)}€</p></div>
                ...
                <motion.div variants={item} className="h-full relative">
                    <div className={`absolute inset-0 border-0 ${cardGlass} z-0`} />
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        <div><p className="text-xs font-bold uppercase text-purple-500 mb-1 tracking-wider">{t.analytics?.kpi.savings || "Épargne"}</p><p className={`text-3xl font-black ${isBlurred ? 'blur-md select-none' : ''}`}>{stats.savings_rate.toFixed(1)}%</p></div>
                        ...
                        <CardContent className="flex-1 p-6 min-h-0">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={daily_data}>
                                    ...
                                    <Tooltip content={<CustomTooltip />} cursor={{ fill: isLight ? '#f1f5f9' : '#18181b', opacity: 0.5 }} />
                                    <Bar dataKey="income" fill="#10b981" radius={[4, 4, 0, 0]} name={t.analytics?.kpi.income || "Revenus"} />
                                    <Bar dataKey="expense" fill="#f43f5e" radius={[4, 4, 0, 0]} name={t.analytics?.kpi.expense || "Dépenses"} />
                                </BarChart>
                            </ResponsiveContainer>
                        </CardContent>
                    </div>
                        ...
                        <CardContent className="flex-1 p-6 flex flex-col min-h-0">
                            ...
                            <div className="flex-1 overflow-y-auto no-scrollbar mt-4">
                                <div className="flex flex-col gap-3">
                                    {category_data.map((entry: any, index: number) => (
                                        <div key={entry.name} className={`flex flex-col gap-2 p-3 rounded-xl border ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-white/5 border-white/5'}`}>
            ...
            <motion.div variants={item} className="relative">
                ...
                <div className="relative z-10">
                    <CardHeader className="pt-8 px-8 pb-4"><CardTitle className="text-sm uppercase tracking-widest opacity-50">{t.analytics?.topExpenses || "Top Dépenses"}</CardTitle></CardHeader>
                    <CardContent className="px-8 pb-8">
                        <div className={`grid grid-cols-1 gap-2 ${isBlurred ? 'blur-sm select-none transition-all duration-500' : ''}`}>
