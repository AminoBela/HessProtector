...
import { Settings, Save, Check, Loader2, Shield, User, FileText, Download, Trash2, Lock } from "lucide-react"
...
export function SettingsView({ settingsForm, setSettingsForm, updateSettings, language, theme, token, logout }: SettingsViewProps) {
    const isLight = theme === 'light';
    const t = Translations[language as keyof typeof Translations].settings as unknown as SettingsTranslations;
    ...
    const handleSaveLimit = async () => {
        ...
        try {
            ...
            setLimits(prev => {
                ...
                return [...prev, { category: limitForm.category, amount: parseFloat(limitForm.amount) }];
            });
    ...
    const handleChangePassword = async () => {
        ...
        try {
            await AccountService.changePassword({ old_password: pwdForm.old, new_password: pwdForm.new }, token);
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show">
            <Card className={`border-0 ${cardGlass} rounded-3xl overflow-hidden min-h-[600px] flex flex-col`}>
                <CardContent className="p-0 flex-1 flex flex-col">
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
                        <div className={`px-8 py-6 border-b ${isLight ? 'border-slate-100' : 'border-white/5'}`}>
                            <TabsList className={`h-auto p-1 bg-transparent gap-3 flex-wrap justify-start`}>
                        ...
                        <div className="p-8 flex-1 overflow-y-auto">
                            <TabsContent value="general" className="mt-0 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="grid grid-cols-1 gap-6">
                                    <div className={sectionBoxStyle}>
                                        <h3 className={`text-sm font-bold uppercase tracking-wider mb-4 opacity-70 flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-white'}`}>
                                            <Settings className="w-4 h-4" /> Préférences
                                        </h3>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            <div className="flex flex-col gap-3">
                                                <label className="text-sm font-semibold opacity-80 ml-1">{t.general.supermarket}</label>
                                                <Input
                                            ...
                                            <div className="flex flex-col gap-3">
                                                <label className="text-sm font-semibold opacity-80 ml-1">{t.general.diet}</label>
                                ...
                                <div className="flex justify-end pt-4">
                                    ...
                                    >
                                        ...
                                        {saveStatus === 'saving' ? "Enregistrement..." : saveStatus === 'success' ? "Enregistré !" : t.general.save}
                                    </Button>
                                </div>
                            </TabsContent>

                            <TabsContent value="account" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="max-w-2xl mx-auto space-y-6">
                                    <div className={sectionBoxStyle}>
                                        <div className="flex flex-col gap-2">
                                            <label className="text-sm font-bold uppercase tracking-wider opacity-60 ml-1">{t.account.email}</label>
                                            <Input value="user@example.com" disabled className={`${inputStyle} opacity-70 cursor-not-allowed`} />
                                        </div>
                                    </div>
                                    ...
                                    <div className={sectionBoxStyle}>
                                        ...
                                        <div className="space-y-4">
                                            <div className="flex flex-col gap-2">
                                                <label className="text-sm font-medium opacity-80 ml-1">{t.account.password}</label>
                                                <Input
                                                    type="password"
                                                    className={inputStyle}
                                                    placeholder="••••••••"
                                            ...
                                            <div className="flex flex-col gap-2">
                                                <label className="text-sm font-medium opacity-80 ml-1">{t.account.newPassword}</label>
                                                <div className="flex gap-3">
                                                    ...
                                                    <Button
                                                        ...
                                                        disabled={pwdStatus === 'loading' || !pwdForm.old || !pwdForm.new}
                                                    ...
                                                    >
                                                        {pwdStatus === 'loading' ? <Loader2 className="w-5 h-5 animate-spin" /> : pwdStatus === 'success' ? <Check className="w-5 h-5" /> : "OK"}
                                                    </Button>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className={`p-6 rounded-2xl border flex flex-col md:flex-row items-center justify-between gap-6 transition-colors ${isLight ? 'bg-rose-50 border-rose-100 hover:border-rose-200' : 'bg-rose-950/20 border-rose-900/30 hover:border-rose-800/50'}`}>
                                        <div className="text-rose-600 dark:text-rose-400">
                                            <p className="font-bold flex items-center gap-2 text-lg">
                                                <Trash2 className="w-5 h-5" />
                                                {t.account.delete}
                                            </p>
                                            <p className="opacity-80 text-sm mt-1">{t.account.deleteDesc}</p>
                                        </div>
                                        <Button
                                            variant="destructive"
                            ...
                            <TabsContent value="legal" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="grid grid-cols-1 gap-6 max-w-4xl mx-auto">
                                    <div className="text-center mb-4">
                                        <h3 className={`text-xl font-black uppercase tracking-tight mb-2 ${isLight ? 'text-slate-800' : 'text-white'}`}>Données & Conformité</h3>
                                        <p className={`text-sm ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>Gérez vos données personnelles et consultez nos documents légaux.</p>
                                    </div>
                                    ...
                                    `}>
                                            ...
                                            <div className="relative z-10">
                                                <div className={`w-14 h-14 rounded-2xl mb-6 flex items-center justify-center transition-transform duration-500 group-hover:scale-110 ${isLight ? 'bg-emerald-100 text-emerald-600' : 'bg-emerald-500/20 text-emerald-400'}`}>
                                                    <Download className="w-7 h-7" />
                                                </div>
                                                <h3 className="font-bold text-2xl mb-2">{t.legal.export}</h3>
                                                <p className={`text-sm leading-relaxed ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>{t.legal.exportDesc}</p>
                                            </div>
                                                ...
                                                >
                                                    {exportStatus === 'loading' ? (
                                                        <span className="flex items-center gap-2">
                                                            <Loader2 className="w-5 h-5 animate-spin" /> Exportation...
                                                        </span>
                                                    ) : (
                                        ...
                                        <div className="flex flex-col gap-4">
                                            <LegalModal
                                                ...
                                                `}>
                                                        <div className="flex items-center gap-5">
                                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${isLight ? 'bg-blue-50 text-blue-600 group-hover:bg-blue-100' : 'bg-blue-500/10 text-blue-400 group-hover:bg-blue-500/20'}`}>
                                                                <Shield className="w-6 h-6" />
                                                            </div>
                                                            <div>
                                                                <span className="font-bold text-lg block mb-1">{t.legal.privacy}</span>
                                                                <span className={`text-xs font-medium uppercase tracking-wider ${isLight ? 'text-blue-600/60' : 'text-blue-400/60'}`}>Document Légal</span>
                                                            </div>
                                                        </div>
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 ${isLight ? 'bg-blue-50 text-blue-600' : 'bg-blue-500/20 text-blue-400'}`}>
                                            ...
                                            <LegalModal
                                                ...
                                                `}>
                                                        <div className="flex items-center gap-5">
                                                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${isLight ? 'bg-purple-50 text-purple-600 group-hover:bg-purple-100' : 'bg-purple-500/10 text-purple-400 group-hover:bg-purple-500/20'}`}>
                                                                <FileText className="w-6 h-6" />
                                                            </div>
                                                            <div>
                                                                <span className="font-bold text-lg block mb-1">{t.legal.terms}</span>
                                                                <span className={`text-xs font-medium uppercase tracking-wider ${isLight ? 'text-purple-600/60' : 'text-purple-400/60'}`}>Conditions</span>
                                                            </div>
                                                        </div>
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 ${isLight ? 'bg-purple-50 text-purple-600' : 'bg-purple-500/20 text-purple-400'}`}>
                                                            <FileText className="w-5 h-5" />
                                                        </div>
                                                    </button>
                                                }
                            ...
                            <TabsContent value="budget" className="mt-0 space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                                <div className="grid grid-cols-1 gap-6">
                                    <div className={sectionBoxStyle}>
                                        <h3 className={`text-sm font-bold uppercase tracking-wider mb-4 opacity-70 flex items-center gap-2 ${isLight ? 'text-slate-900' : 'text-white'}`}>
                                        ...
                                        <div className="flex gap-4 items-end mb-6">
                                            ...
                                            >
                                                {limitStatus === 'saving' ? <Loader2 className="w-5 h-5 animate-spin" /> : <Check className="w-5 h-5" />}
                                            </Button>
                                        </div>

                                        <div className="space-y-3">
                                            {limits.map((l, i) => (
                                                <div key={i} className={`flex justify-between items-center p-4 rounded-xl border ${isLight ? 'bg-white border-slate-100' : 'bg-black/20 border-white/5'}`}>
                                                    <span className="font-bold">{l.category}</span>
                                                    <span className="font-mono text-emerald-500 font-bold">{l.amount}€</span>
                                                </div>
                                            ))}
                                            {limits.length === 0 && <div className="text-center opacity-50 text-sm py-4">Aucune limite définie.</div>}
                                        </div>
                                    </div>
                                </div>
                            </TabsContent>
