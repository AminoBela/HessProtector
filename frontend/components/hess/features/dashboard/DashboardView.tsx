...
export function DashboardView({ data, barData, COLORS, language, theme, statsData, years, statsYear, setStatsYear, t }: DashboardViewProps) {
    ...
    const CustomTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            return (
                <div className={`p-4 rounded-2xl border backdrop-blur-xl shadow-2xl ${isLight ? 'bg-white/90 border-emerald-900/10' : 'bg-black/80 border-white/10'}`}>
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="space-y-8">
            <div className="flex justify-between items-center mb-6">
                <div>
                    {data.safe_balance < 50 && <PanicModal language={language} isLight={isLight} />}
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <motion.div variants={item} className="md:col-span-2 relative min-h-[320px] flex flex-col justify-between">
                    <div className={`absolute inset-0 border-0 ${cardGlass} z-0`} />
                    <div className={`absolute top-0 right-0 w-[500px] h-[500px] rounded-full blur-[120px] -mr-32 -mt-32 pointer-events-none mix-blend-screen z-0 ${isLight ? 'bg-emerald-500/10' : 'bg-emerald-500/20'}`}></div>
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        ...
                        <CardContent className="p-0 mt-6">
                            <div className="flex items-baseline gap-2">
                                <span className={`text-6xl md:text-8xl font-black tracking-tighter ${bigNumberColor} ${isBlurred ? 'blur-xl select-none' : ''}`}>{statsData ? statsData.net_result.toFixed(2) : '...'}</span>
                            ...
                            <div className="mt-8 flex flex-col md:flex-row gap-4 text-sm font-bold">
                                <div className={`px-5 py-3 rounded-2xl border flex items-center gap-3 ${pillBg} ${pillText}`}>
                                    <TrendingUp className="w-5 h-5 text-emerald-500" /> {t.incoming}: <span className={isBlurred ? 'blur-sm' : ''}>{statsData ? statsData.total_income.toFixed(2) : 0}€</span>
                                </div>
                                <div className={`px-5 py-3 rounded-2xl border flex items-center gap-3 ${pillBg} ${pillText}`}>
                                    <Zap className="w-5 h-5 text-rose-500" /> {t.outgoing}: <span className={isBlurred ? 'blur-sm' : ''}>{statsData ? statsData.total_expense.toFixed(2) : 0}€</span>
                                </div>
                ...
                <motion.div variants={item} className="md:col-span-1 relative min-h-[320px] flex flex-col justify-between">
                    <div className={`absolute inset-0 border-0 ${cardGlass} rounded-3xl z-0`}></div>
                    <div className="relative z-10 p-6 flex flex-col justify-between h-full">
                        <div className="flex justify-between items-start">
                            <div>
                                <h3 className={`text-sm font-bold uppercase tracking-widest mb-1 ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>{t.safeBalance}</h3>
                                <div className={`text-5xl md:text-7xl font-black tracking-tighter ${isBlurred ? 'blur-md select-none' : ''}`}>
                                    {data?.safe_balance?.toFixed(2)}€
                                </div>
                            </div>
                            <div className={`p-3 rounded-full ${isLight ? 'bg-emerald-100/50 text-emerald-600' : 'bg-emerald-500/10 text-emerald-400'}`}>
                                <TrendingUp className="w-8 h-8" />
                            </div>
                        </div>

                        <div className={`mt-8 p-4 rounded-2xl border ${isLight ? 'bg-indigo-50/50 border-indigo-100' : 'bg-indigo-500/10 border-indigo-500/10'}`}>
                            <div className="flex items-center gap-3">
                                <div className={`p-2 rounded-full ${isLight ? 'bg-indigo-100 text-indigo-600' : 'bg-indigo-500/20 text-indigo-400'}`}>
                                    <Zap className="w-4 h-4" />
                                </div>
                                <div className="flex-1">
                                    <p className={`text-xs font-bold uppercase ${isLight ? 'text-indigo-400' : 'text-indigo-400'}`}>Reste à vivre (Est.)</p>
                                    <p className={`text-lg font-bold ${isLight ? 'text-indigo-900' : 'text-indigo-100'}`}>{data?.prediction?.projected_end}€</p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <motion.div variants={item} className="h-[400px] relative">
                    ...
                    <div className="relative z-10 h-full">
                        ...
                        <CardContent className={`h-[300px] ${isBlurred ? 'blur-lg transition-all duration-500' : ''}`}>
                            {statsData ? (
                                ...
                                <div className="space-y-3">
                                    {data.transactions.slice(0, 5).map((tx: any) => (
                                        <div key={tx.id} className={`flex justify-between items-center p-5 rounded-2xl border transition-all ${itemBg}`}>
                                            ...
                                            <span className={`font-mono text-xl font-bold ${tx.type === 'revenu' ? 'text-emerald-500' : 'text-rose-500'} ${isBlurred ? 'blur-sm select-none' : ''}`}>{tx.type === 'revenu' ? '+' : '-'}{parseFloat(tx.amount).toFixed(2)}€</span>
                                        </div>
                                    ))}
                                </div>
                            </ScrollArea>
...

            {
                data.achievements && (
                    <motion.div variants={item}>
                        <h3 className={`text-xs font-bold uppercase tracking-widest mb-6 ${subTextColor}`}>🏆 {t.achievements || "Salle des Trophées"}</h3>
                        <AchievementsView achievements={data.achievements} isLight={isLight} language={language} />
                    </motion.div>
                )
            }
        </motion.div >
