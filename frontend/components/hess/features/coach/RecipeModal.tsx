...
interface RecipeModalProps {
    isOpen: boolean;
    onClose: () => void;
    recipe: any; // JSON generated by AI
    loading: boolean;
    language: string;
}

export const RecipeModal: React.FC<RecipeModalProps> = ({ isOpen, onClose, recipe, loading, language }) => {
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
                ...
                >
                    ...
                    <h3 className="text-xl font-bold text-white mb-2">
                        {language === 'fr' ? "Le Chef réfléchit..." : "The Chef is cooking..."}
                    </h3>
                    <p className="text-zinc-400 text-sm">
                        {language === 'fr' ? "Analyse du frigo et création de la recette parfaite." : "Checking the fridge and crafting the perfect recipe."}
                    </p>
                </motion.div>
            </div>
        );
                ...
                <div className="p-6 border-b border-zinc-800 flex justify-between items-start bg-zinc-900 z-10">
                    <div>
                        <h2 className="text-3xl font-bold text-white flex items-center gap-3">
                            <span className="text-4xl">{recipe.title?.split(' ')[0]}</span> {/* Emoji assumption */}
                            <span>{recipe.title?.replace(/^[^\s]+\s/, '') || "Recette"}</span>
                        </h2>
                        <div className="flex gap-4 mt-3 text-sm font-medium text-emerald-400">
                            {recipe.time && <div className="flex items-center gap-1"><Clock className="w-4 h-4" /> {recipe.time}</div>}
                            {recipe.difficulty && <div className="flex items-center gap-1"><Dumbbell className="w-4 h-4" /> {recipe.difficulty}</div>}
                            {recipe.calories && <div className="flex items-center gap-1"><Flame className="w-4 h-4" /> {recipe.calories}</div>}
                        </div>
                    </div>
                    <button
                        onClick={onClose}
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            {recipe.ingredients?.map((ing: any, i: number) => (
                                <div key={i} className="flex items-center justify-between bg-zinc-950 p-3 rounded-xl border border-zinc-800">
                                    <span className="text-zinc-200 font-medium">{ing.item}</span>
                                    <div className="text-right">
                                        <span className="block text-emerald-400 text-sm font-bold">{ing.qty}</span>
                                        {ing.substitution && (
                                            <span className="block text-xs text-amber-500 italic">Example: {ing.substitution}</span>
                                        )}
                                    </div>
                                </div>
                            ))}
                ...
                {recipe.chef_tip && (
                    <div className="p-4 bg-emerald-900/10 border-t border-emerald-500/20">
                        <div className="flex items-start gap-3">
                            ...
                            <div>
                                <h4 className="text-xs font-bold text-emerald-500 uppercase tracking-wider mb-1">Chef's Tip</h4>
                                <p className="text-emerald-300 text-sm italic">"{recipe.chef_tip}"</p>
                            </div>
                        </div>
                    </div>
                )}
