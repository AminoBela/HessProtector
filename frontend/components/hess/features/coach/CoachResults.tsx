...
import { Lightbulb, Loader2, ArrowLeft, Check, PenSquare, RefreshCw, Save, ShoppingCart, ChefHat } from "lucide-react"
...
export function CoachResults({
    ...
    isEditing, setIsEditing, planMeals, token
}: CoachResultsProps) {
    ...
    const updateMeal = (index: number, type: 'lunch' | 'dinner', value: string) => {
    ...
    const handleRecipeClick = async (dish: string) => {
        ...
        // Caching: Check if we already have it
        if (recipeCache[dish]) {
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 lg:grid-cols-3 gap-8 pb-20">
            <RecipeModal
                isOpen={isRecipeOpen}
            ...
            <div className="lg:col-span-2 space-y-6 bg-transparent">
                ...
                <Dialog open={isSaveDialogOpen} onOpenChange={setIsSaveDialogOpen}>
                    <DialogContent className={isLight ? "bg-white/95 backdrop-blur-xl border-emerald-900/10 text-slate-800" : "bg-zinc-950 border-white/10 text-white"}>
                        <DialogHeader>
                        ...
                        <div className="py-4">
                            <Input
                                ...
                                className={isLight ? "bg-white border-emerald-900/10 text-slate-800" : "bg-zinc-900 border-white/10 text-white"}
                            />
                        </div>
                        <DialogFooter>
                ...
                <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 gap-4 relative">
                                ...
                                {shouldShowMeal(day.lunch, 'lunch') && (
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            {!isEditing && <ChefHat className="w-3 h-3 text-zinc-500 group-hover:text-orange-400 transition-colors opacity-0 group-hover:opacity-100" />}
                                        </div>
                                        {isEditing ? (
                                ...
                                {shouldShowMeal(day.dinner, 'dinner') && (
                                    ...
                                    >
                                        <div className="flex justify-between items-start">
                                            ...
                                            {!isEditing && <ChefHat className="w-3 h-3 text-zinc-500 group-hover:text-indigo-400 transition-colors opacity-0 group-hover:opacity-100" />}
                                        </div>
                                        {isEditing ? (
                                            <div onClick={(e) => e.stopPropagation()}>
                                                <Input
