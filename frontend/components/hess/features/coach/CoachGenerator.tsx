...
export function CoachGenerator({
    ...
    onGenerate, onShowSaved,
    updating, language, theme, diet
}: CoachGeneratorProps) {
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 gap-8 h-[calc(100vh-180px)] items-center">
            <Card className={`border-0 ${cardGlass} flex flex-col justify-center p-12 h-full relative`}>
                <Button variant="ghost" onClick={onShowSaved} className="absolute top-6 right-6 text-zinc-500 hover:text-white hover:bg-white/10 rounded-full h-10 w-10 p-0 md:w-auto md:px-4 md:h-10">
                    <History className="w-5 h-5 md:mr-2" /> <span className="hidden md:inline">{t.savedTitle}</span>
                </Button>
                ...
                <div className="mb-8 text-center">
                    ...
                    <h3 className="text-3xl font-black uppercase tracking-tighter mb-2">{t.title}</h3>
                    <p className={`text-lg ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>{t.subtitle(diet)}</p>
                </div>
                ...
                <div className="space-y-8">
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={`font-mono text-xl font-bold ${isLight ? 'text-slate-700' : 'text-white'}`}>{planDays[0]} {common.days}</span>
                        </div>
                    ...
                    <Button onClick={onGenerate} disabled={updating} className={`w-full h-16 text-xl font-bold rounded-2xl shadow-xl transition-all ${isLight ? 'bg-white text-black hover:bg-zinc-200 border border-zinc-200' : 'bg-emerald-600 text-white hover:bg-emerald-500'}`}>
                        {updating ? <Loader2 className="w-6 h-6 animate-spin mr-2 text-emerald-600" /> : <Sparkles className="mr-2 w-5 h-5 text-emerald-600" />} {updating ? common.generating : t.generate}
                    </Button>
                </div>
            </Card>
            <div className="hidden md:flex flex-col items-center justify-center p-10 text-center opacity-40">
                ...
                <p className={`text-lg font-light max-w-xs ${isLight ? 'text-slate-500' : 'text-zinc-400'}`}>{t.accessSaved}</p>
            </div>
        </motion.div>
    );
}
