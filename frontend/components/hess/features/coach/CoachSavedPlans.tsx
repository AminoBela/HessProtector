...
export function CoachSavedPlans({ savedPlans, onBack, onLoad, onDelete, language, theme }: CoachSavedPlansProps) {
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show" className="h-[calc(100vh-180px)]">
            <div className="flex items-center gap-4 mb-6">
                <Button variant="ghost" onClick={onBack} className="text-zinc-400 hover:text-white hover:bg-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {safeSavedPlans.map(plan => (
                    <div key={plan.id} className={`${cardGlass} p-5 group relative flex flex-col`}>
                        ...
                        <div className="flex gap-2 mt-2">
                            <Button onClick={() => onLoad(plan)} className="flex-1 bg-emerald-600 hover:bg-emerald-500 h-9 text-xs font-bold">{t.load}</Button>
